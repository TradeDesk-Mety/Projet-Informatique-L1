...
def backtest_strategy(prices: pd.Series, strategy_type: str = "SMA", params: dict = None, df_ohlcv: pd.DataFrame = None) -> dict:
    ...
    if strategy_type == "SMA":
        ...
        threshold   = params.get("threshold", 1.5) if params else 1.5  
    ...
    #Rendement passif pour comparer
...
def monte_carlo_simulation(prices: pd.Series, days_to_simulate: int = 30, num_simulations: int = 100) -> dict:
    """
    Exécute une simulation de Monte-Carlo basée sur le Mouvement Brownien Géométrique 
    ...
    #Calcul de la dérive et de la volatilité
    mu = returns.mean()
    sigma = returns.std()
