...
st.title("📐 Options — Pricing Black-Scholes & Greeks")
st.caption("Modélisation 3D du prix d'une option européenne (Call / Put) selon le modèle Black-Scholes (1973).")
...
try:
    ...
    c1.metric(f"Prix {opt_type}", f"{price:.4f} $")
    c2.metric("Delta (Δ)",  f"{g['delta_call']:.4f}" if opt_key == "call" else f"{g['delta_put']:.4f}")
    c3.metric("Gamma (Γ)",  f"{g['gamma']:.4f}")
    c4.metric("Theta (Θ)",  f"{g['theta_call']:.4f}" if opt_key == "call" else f"{g['theta_put']:.4f}")
    c5.metric("Vega (ν)",   f"{g['vega']:.4f}")
    c6.metric("Rho (ρ)",    f"{g['rho_call']:.4f}" if opt_key == "call" else f"{g['rho_put']:.4f}")
...
# ── Onglets ───────────────────────────────────────────────────────────────────
tab_3d, tab_smile, tab_sens = st.tabs(["🌐 Surface 3D", "😊 Volatility Smile", "📊 Sensibilité"])
...
with tab_3d:
    ...
    **Lecture de la surface :**
    ...
    - **Axe Y** — Maturité T : une option lointaine vaut plus (valeur temps).
...
with tab_smile:
    ...
    for vol_s, col_s in zip(vols_smile, palette):
        prices_smile = [
            grk.black_scholes_pricing(S, k, T, r, vol_s, opt_key)
    ...
    fig_smile.update_layout(
        title=f"Prix du {opt_type} en fonction du Strike K — Volatility Smile",
...
with tab_sens:
    ...
    **Rappel des Greeks :**
    ...
    | **Γ Gamma** | Variation du Delta (courbure) |
    | **ν Vega** | Sensibilité à la volatilité |
    | **Θ Theta** | Perte de valeur par jour (décroissance temporelle) |
    """)
